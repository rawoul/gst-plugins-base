...
/* automatic sizes, while prerolling we buffer up to 4MB, we ignore time
 * and buffers in this case. */
#define AUTO_PREROLL_SIZE_BYTES                  4 * 1024 * 1024
...
/* when playing, keep a max of 4MB of data but try to keep the number of buffers
 * as low as possible (try to aim for 5 buffers) */
#define AUTO_PLAY_SIZE_BYTES        4 * 1024 * 1024
